...
## 8. Script hỗ trợ (tạo role mẫu, hướng dẫn RLS/CLS)
Chạy lệnh sau để tạo sẵn các role phòng ban và xem hướng dẫn tạo RLS/CLS qua API:
```powershell
...
## 9. Gặp lỗi? Xử lý thế nào?
- Không tạo được admin: Kiểm tra biến `ADMIN_*` trong `.env` và xem log Superset.
- Lỗi DB: Kiểm tra biến `POSTGRES_*` và volume `db_data`.
- Đổi cấu hình không nhận: Sửa file config, restart lại container.
- Muốn vào shell Superset:
```powershell
...
- [Superset Docker image](https://hub.docker.com/r/apache/superset)
- [Hướng dẫn RLS](https://superset.apache.org/docs/security/row-level-security)
- [Hướng dẫn OIDC/LDAP](https://superset.apache.org/docs/installation/security)
